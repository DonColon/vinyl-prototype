...
class VanillaSlider extends HTMLElement {
    ...
    constructor() {
        ...
        if (!this.shadowRoot) {
            ...
            this.showRange = (this.getAttribute('showRange') === 'true');
        }
    }
    ...
    slide(event) {
        if (this.isSliding) {
            ...
            const percentage = ratio.toFixed(2) * 100;
            const value = Number(this.max) * ratio + Number(this.min);
    ...
    slideTo(percentage) {
        ...
        if (this.showRange) this.sliderRange.style.width = `${percentage}%`;
    }
